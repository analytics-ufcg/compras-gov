...
//
    ...
    function ready(err, strikeouts, teams) {
        ...
        function drawLegend(container, headline) {
            ...
            teamChooser.on("change", function () {
                ...
                .attr("label", function (d) {
                    return ((d.key === "NL" ? "National" : "American") + " League").toUpperCase();
                })
                .selectAll("option")
                .data(function (d) {
                    return d.values;
        ...
        function drawLineChart(container, data, attributeY, x, y, width, height, r, bounds) {
            ...
            teamsContainer.append("g")
                ...
                .selectAll(".voronoi")
                .data(d3.geom.voronoi(data.map(function (d) {
                    return [x(d.year), y(d[attributeY]) + Math.random() - .5];
                })).map(bounds.clip).map(function (d, i) {
                    d.path = "M" + d.join("L") + "Z";
                    d.data = data[i];
                    return d;
                }))
